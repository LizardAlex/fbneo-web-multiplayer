var fbneo = (() => {
	...
	return async function (fbneo = {}) {
		...
		var wasmBinaryFile;
		if (Module["locateFile"]) {
			wasmBinaryFile = "fbneo-konami.wasm";
			if (!isDataURI(wasmBinaryFile)) {
				wasmBinaryFile = locateFile(wasmBinaryFile);
			}
		} else {
			wasmBinaryFile = new URL("fbneo-konami.wasm", import.meta.url).href;
		}
		...
		var ASM_CONSTS = {
			...
			1049265: ($0, $1, $2, $3) => {
				...
				SDL2.audio.scriptProcessorNode["onaudioprocess"] = function (
					e
				) {
			...
			1049675: ($0, $1) => {
				var SDL2 = Module["SDL2"];
				var numChannels =
					SDL2.capture.currentCaptureBuffer.numberOfChannels;
			...
			1050280: ($0, $1) => {
				var SDL2 = Module["SDL2"];
				var numChannels =
					SDL2.audio.currentOutputBuffer["numberOfChannels"];
				for (var c = 0; c < numChannels; ++c) {
					var channelData =
		...
		function registerRestoreOldStyle(canvas) {
			...
			var oldDocumentBackgroundColor =
		...
		function _glGetShaderPrecisionFormat(
			...
			var result = GLctx.getShaderPrecisionFormat(
				shaderType,
				precisionType
			);
		...
		function _glGetShaderSource(shader, bufSize, length, source) {
			...
			var numBytesWrittenExclNull =
				bufSize > 0 && source
					? stringToUTF8(result, source, bufSize)
					: 0;
			if (length) HEAP32[length >> 2] = numBytesWrittenExclNull;
		}
				...
				for (var i = 0; i < data.length; i++) {
					switch (type) {
						...
						case 2:
							...
							break;
						case 5:
							HEAP32[(params + i * 4) >> 2] = Math.fround(
								data[i]
							);
							break;
					}
		...
		function emscriptenWebGLGetTexPixelData(
			...
			var sizePerPixel =
				colorChannelsInGlTextureFormat(format) * byteSize;
		...
		for (var i = 0; i < 288; ++i) {
			miniTempWebGLIntBuffers[i] =
				miniTempWebGLIntBuffersStorage.subarray(0, i + 1);
		}
		var wasmImports = {
			...
			Ba: _emscripten_exit_fullscreen,
			...
			W: _emscripten_glBlendFuncSeparate,
			V: _emscripten_glBufferData,
			U: _emscripten_glBufferSubData,
			T: _emscripten_glCheckFramebufferStatus,
			S: _emscripten_glClear,
			...
			Nc: _emscripten_glGetFloatv,
			Mc: _emscripten_glGetFramebufferAttachmentParameteriv,
			...
			Bc: _emscripten_glGetTexParameteriv,
			yc: _emscripten_glGetUniformLocation,
			Ac: _emscripten_glGetUniformfv,
			...
			Jb: _emscripten_glUniform2fv,
			Ib: _emscripten_glUniform2i,
			Hb: _emscripten_glUniform2iv,
			Gb: _emscripten_glUniform3f,
			Fb: _emscripten_glUniform3fv,
			Eb: _emscripten_glUniform3i,
			Db: _emscripten_glUniform3iv,
			Cb: _emscripten_glUniform4f,
			Bb: _emscripten_glUniform4fv,
			Ab: _emscripten_glUniform4i,
			yb: _emscripten_glUniform4iv,
			xb: _emscripten_glUniformMatrix2fv,
			...
			I: _emscripten_request_pointerlock,
			Va: _emscripten_resize_heap,
			R: _emscripten_sample_gamepad_data,
			...
			Da: _emscripten_set_window_title,
			l: _emscripten_sleep,
			bb: _environ_get,
			cb: _environ_sizes_get,
			M: _fd_close,
			db: _fd_read,
			Ra: _fd_seek,
			K: _fd_write,
		...
		var ___wasm_call_ctors = function () {
			return (___wasm_call_ctors = Module["asm"]["Id"]).apply(
				null,
				arguments
			);
		};
		...
		var _malloc = function () {
			return (_malloc = Module["asm"]["Kd"]).apply(null, arguments);
		};
		var ___errno_location = function () {
			return (___errno_location = Module["asm"]["Ld"]).apply(
				null,
				arguments
			);
		};
		...
		var _setThrew = function () {
			return (_setThrew = Module["asm"]["_d"]).apply(null, arguments);
		};
		var stackSave = function () {
			return (stackSave = Module["asm"]["$d"]).apply(null, arguments);
		};
		var stackRestore = function () {
			return (stackRestore = Module["asm"]["ae"]).apply(null, arguments);
		};
		var stackAlloc = function () {
			return (stackAlloc = Module["asm"]["be"]).apply(null, arguments);
		};
